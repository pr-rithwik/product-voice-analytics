...
def download_artifacts():
    ...
    if not os.path.exists(str(TFIDF_VECTORIZER_PATH)):
        hf_hub_download(repo_id=HF_MODEL_REPO, filename=TFIDF_VECTORIZER_FILE, local_dir=str(MODELS_DIR), local_dir_use_symlinks=False)

    # LR downloader
    if not os.path.exists(str(LR_MODEL_PATH)):
        hf_hub_download(repo_id=HF_MODEL_REPO, filename=LR_MODEL_FILE, local_dir=str(MODELS_DIR), local_dir_use_symlinks=False)
    ...
    if not os.path.exists(str(PRODUCT_LOOKUP_PATH)):
        hf_hub_download(repo_id=HF_DATASET_REPO, filename='product_lookup.csv', local_dir=str(MODELS_DIR), local_dir_use_symlinks=False)
